...
function acquire_lock(lock_key, lock_value=string(uuid4()); client=get_global_client(), timeout=nothing, seconds_between_checks=0.1)
    while true
        ...
        was_set = !isnothing(execute(["SET", lock_key, lock_value, "NX", px...], JedisCluster.get_client(client, [lock_key], true, false)))
...
function release_lock(lock_key, lock_value; client=get_global_client())::Bool
    if get(lock_key; client=JedisCluster.get_client(client, [lock_key], true, false)) == lock_value
        del(lock_key; client=JedisCluster.get_client(client, [lock_key], true, false))
        return true
    else
        return false
    end
...
function redis_lock(fn::Function, lock_key, lock_value=string(uuid4()); client=get_global_client(), timeout=nothing, seconds_between_checks=0.1)
    acquire_lock(lock_key, lock_value; client=JedisCluster.get_client(client, [lock_key], true, false), timeout=timeout, seconds_between_checks=seconds_between_checks)

    try
        fn()
    finally
        release_lock(lock_key, lock_value; client=JedisCluster.get_client(client, [lock_key], true, false))
    end
end
...
"""
isredislocked(lock_key; client=get_global_client()) = exists(lock_key; client=JedisCluster.get_client(client, [lock_key], true, false)) == 1