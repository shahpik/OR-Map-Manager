...
@testset "Counter Tests" begin
    ...
    tp_k1 = PrometheusClient._get_label_key_val(tp, ["t1", "t2", "t3"])
    ...
    tp_k2 = PrometheusClient._get_label_key_val(tp, tp_kl2)
...
@testset "Float/Value type Tests" begin
    ...
    lfloat1 = PrometheusClient._get_label_key_val(tp_float, lfkey1)
    ...
    lf_label2 = PrometheusClient._get_label_key_val(tp_float, lfkey2)
    inc(tp_float,lfkey2)
...
@testset "Gauge Tests" begin
    ...
    # print(PrometheusClient._prometheus_format(gm))
end


@testset "Formatting Tests" begin
    lab_pai_1 = Set(["host" => "localhost","tid" => "1","podname" => "t2"])
    @test PrometheusClient._prometheus_format_label("testmetric",lab_pai_1, 1.0) == "testmetric{host=\"localhost\",tid=\"1\",podname=\"t2\"} 1.0"
    ...
    @test PrometheusClient._prometheus_format(pm_format) == "# HELP formatter_test format test\n# TYPE formatter_test gauge\nformatter_test{host=\"localhost\"} 1 \n\n"
    ...
    """
    @test PrometheusClient._prometheus_format(pm_format) == formatted_string
    ...
    """
    @test PrometheusClient._prometheus_pretty(pm_format) == pretty_format
    @test PrometheusClient.generate_pretty_prom_metrics([pm_format]) == pretty_format

    # number of lines
    @test count("\n", PrometheusClient._prometheus_format(pm_format)) == 5
end
...
@testset "Collector Tests" begin
    ...
    length(PrometheusClient.DEFAULT_COLLECTOR) == count("TYPE", generate_latest_prom_metrics())  # 4
end
