...
```
StructTypes.StructType(::Type{GQLInterface.get_introspected_object(object_name)}) = StructTypes.Struct()
```
...
# Examples
```julia
julia> GQLInterface.create_introspected_struct("ResultObject",Dict(:resultId=>"MyResult", :Score => 1.0))
ResultObject
  Score : 1.0
      resultId : MyResult
```
