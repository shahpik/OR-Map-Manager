...
# Examples
```jldoctest; setup=:(using GQLInterface: get_query_args_str, prettify_query)
...
# Examples
```jldoctest; setup=:(using GQLInterface: initialise_arg_names)
...
# Examples
```jldoctest; setup=:(using GQLInterface: directly_write_query_args, prettify_query)
...
# Examples
```jldoctest; setup=:(using GQLInterface)
julia> println(GQLInterface.prettify_query("[{(a)}]"))
[
    {
        (
            a
...
julia> println(GQLInterface.prettify_query("[{(a,b),(c,d)}]"))
[
    {
        (
            a
