@testset "GQLLocation" begin
    ...
    loc = JSON3.read(str, GQLInterface.GQLLocation)
    @test loc.line == 1
    @test loc.column == 10
...
@testset "GQLError" begin
    ...
    err = JSON3.read(str, GQLInterface.GQLError)
...
@testset "GQLResponse" begin
    ...
    """
    resp = JSON3.read(response, GQLInterface.GQLResponse{Any})
    @inferred JSON3.read(response, GQLInterface.GQLResponse{Any})
    ...
    resp = JSON3.read(response, GQLInterface.GQLResponse{QueryNameResponse})
    @inferred JSON3.read(response, GQLInterface.GQLResponse{QueryNameResponse})
