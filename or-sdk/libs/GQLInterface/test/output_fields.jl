@testset "get_output_str" begin
    @test GQLInterface.get_output_str("field1") == "field1,"
    @test GQLInterface.get_output_str(Alias("alias_name", "field_name")) == "alias_name:field_name,"
    @test GQLInterface.get_output_str(["field1"]) == "field1,"
    @test GQLInterface.get_output_str(["field1", "field2"]) == "field1,field2,"
    @test GQLInterface.get_output_str(Dict("field1" => "field2")) == "field1{field2,},"
    @test GQLInterface.get_output_str(Dict("field1" => ["field2"])) == "field1{field2,},"
    @test GQLInterface.get_output_str(Dict("field1" => ["field2", "field3"])) == "field1{field2,field3,},"
    @test GQLInterface.get_output_str(["field1", Dict("field2" => ["field3"])]) == "field1,field2{field3,},"
end
...
@testset "_get_fields_str" begin
    ...
    @test GQLInterface._get_field_str(client, field, [], []) == "object_field_name{field1,field2,}"

    # List of scalars
    field = build_type(nothing, "scalar_field_name",
        type=build_type("LIST", nothing,
            ofType=build_type("SCALAR", "String")))
    @test GQLInterface._get_field_str(client, field, [], []) == "scalar_field_name,"
    ...
    @test GQLInterface._get_field_str(client, field, [], []) == "enum_field_name,"
    field = build_type(nothing, "enum_field_name",
        type=build_type("NON_NULL", nothing,
    ...
    @test GQLInterface._get_field_str(client, recursive_object, [], objects_that_recurse) == "object_field_name{field1,}"
    ...
    @test_throws GQLInterface.GraphQLClientException GQLInterface._get_field_str(client, field, [], [])
...
@testset "get_all_output_fields_str" begin
    ...
    @test GQLInterface.get_field_names_string(client, type, [], []) == "field1,field2{field2,}"

    # Type with no fields
    type = "MyEmptyType"
    client.type_to_fields_map[type] = Dict()
    @test GQLInterface.get_field_names_string(client, type, [], []) == ""

    # get_all_output_fields_str
    query_name = "myQuery"
    client.query_to_type_map[query_name] = "MyType"
    @test GQLInterface.get_all_output_fields_str(client, query_name, []) == "field1,field2{field2,}"
    query_name = "myQuery"
    client.query_to_type_map[query_name] = "MyEmptyType"
    @test GQLInterface.get_all_output_fields_str(client, query_name, []) == ""
    ...
    @test GQLInterface.get_all_output_fields_str(client, query_name, []) == "field1,"
    @test_logs (:warn, Regex("Can't query all output fields due to recursion of these object\\(s\\):")) GQLInterface.get_all_output_fields_str(client, query_name, [])
end