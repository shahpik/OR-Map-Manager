...
@testset "timers" begin
    ...
    HTTP.WebSockets.open("ws://127.0.0.1:8080") do ws
        # timeout
        ch = GQLInterface.async_reader_with_timeout(ws, 0.1)
        @test take!(ch) == :timeout

        ch = GQLInterface.async_reader_with_timeout(ws, 5)
        write(ws, "Data")
        ...
        stop = Ref(false)
        stopfn() = stop[] 
        ch = GQLInterface.async_reader_with_stopfn(ws, stopfn, 0.5)
        @test !isready(ch)
        stop[] = true
        sleep(1.0)
        @test take!(ch) == :stopfn
        ...
        write(ws, "Data")
        @test String(GQLInterface.readfromwebsocket(ws, nothing, 0)) == "Data"

        # readfromwebsocket - timeout
        @test GQLInterface.readfromwebsocket(ws, nothing, 0.1) == :timeout
        ...
        @test GQLInterface.readfromwebsocket(ws, stopfn2, 0.1) == :stopfn
    end
end
...
@testset "subscriptions.jl" begin
    ...
    @test_throws GQLInterface.GraphQLError open_subscription(()->(), client, "Doesn't Exist")
    ...
    @test results[1] isa GQLInterface.GQLResponse
    ...
    @test_throws GQLInterface.GraphQLError open_subscription(
        (val)->(push!(results, val); true),
        client,
        "MySub",
        output_fields="field",
    ...
    open_subscription(
        ...
        Response,
        output_fields="field")
    @test results[1] isa GQLInterface.GQLResponse{Response}
    @test isnothing(results[1].errors)
    @test !isnothing(results[1].data) # No point testing content as we've coded it in the test function
end