@testset "initialise_arg_names" begin
    ...
    initialised_dict = GQLInterface.initialise_arg_names(test_dict)
...
@testset "get_query_args_str" begin
    ...
    str, arg_names = GQLInterface.get_query_args_str(args)
...
@testset "directly_write_query_args" begin
    @test GQLInterface.directly_write_query_args(Dict("a"=>1)) == "a:1"
    @test GQLInterface.directly_write_query_args(Dict("a"=>"string")) == "a:\"string\""
    @test GQLInterface.directly_write_query_args(Dict("a"=>GQLEnum("enum"))) == "a:enum"
    @test GQLInterface.directly_write_query_args(Dict("a"=>true)) == "a:true"
    @test GQLInterface.directly_write_query_args(Dict("a"=>[1])) == "a:[1]"
    str = GQLInterface.directly_write_query_args(Dict("a"=>1, "b"=>1.0))
    @test occursin("a:1", str)
    @test occursin("b:1.0", str)
    @test GQLInterface.directly_write_query_args(Dict("a"=>Dict("b"=>[1]))) == "a:{b:[1]}"
    @test GQLInterface.directly_write_query_args(Dict()) == ""
end