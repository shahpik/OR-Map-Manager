...
has happened during the execution of the operation. GQLInterface, however, will always
...
Whether or not GQLInterface throws an exception when an error occurs during execution
...
GQLInterface uses [JSON3](https://github.com/quinnj/JSON3.jl) to deserialize the response.
...
DocTestSetup = quote
    ...
    using GQLInterface: GQLResponse
end
```
...
If a custom type is used and deserialisation fails, GQLInterface will attempt to deserialize
...
└ @ GQLInterface ../GQLInterface/src/http_execution.jl:144
```
...
└ @ GQLInterface ../GQLInterface/src/http_execution.jl:150
...
If the `retry` keyword argument is `true`, GQLInterface will retry the opening of the subscription if it fails. This keyword argument is passed directly to `HTTP.WebSockets.open`.
