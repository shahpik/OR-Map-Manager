...
JSON is the most common serialization format for GraphQL servers, and GQLInterface assumes a
JSON response. As explained in [Custom Types](@ref custom_type_ref), GQLInterface deserialises
...
julia> print(GQLInterface.prettify_query(response_str))
{
    "data":{
        "MyQuery":{
            "field1":1
...
In cases where this is due to an error from the server, then GQLInterface will attempt to deserialise with `Any`
