...
In this case, GQLInterface used the introspected schema to determine what output fields
...
GQLInterface.GQLResponse{Vector{CountryName}}
...
```julia-repl
julia> Country = GQLInterface.introspect_object(client, "Country")
...
└ @ GQLInterface ../GQLInterface/src/type_construction.jl:75
GQLInterface.var"##Country#604"

julia> response = query(client, query_alias, Vector{Country}, query_args=query_args, output_fields="name")
GQLResponse{Vector{GQLInterface.var"##Country#604"}}
  data: Dict{String, Union{Nothing, Vector{GQLInterface.var"##Country#604"}}}
          country_names: Vector{GQLInterface.var"##Country#604"}
...
GraphQL is often used with microservice architectures. Often, you will find that you have multiple microservices that perform the same GraphQL queries. A nice solution to this is to write a new package which wraps GQLInterface and provides a higher-level interface, and which also handles connection to the server. For example, the `country` query above could be wrapped as follows
