...
GQLInterface.GQLResponse{GQLInterface.var"##Country#261"}
  data: Dict{String, Union{Nothing, GQLInterface.var"##Country#261"}}
          country: GQLInterface.var"##Country#261"
...
GQLInterface keeps track of all introspected objects so that if two fields use the same object,
...
GQLInterface also keeps track of the object(s) that is/are currently being introspected to 
...
└ @ GQLInterface ../GQLInterface/src/type_construction.jl:75
...
GQLInterface cannot introspect these fields, otherwise we would be using the type that we
...
└ @ GQLInterface ../GQLInterface/src/type_construction.jl:78
...
ERROR: MethodError: Cannot `convert` an object of type GQLInterface.var"##Language#404" to an object of type GQLInterface.var"##Language#400"
```
...
julia> country = create_introspected_struct(client, "Country", Dict("languages" => [language]))
Country
  languages : GQLInterface.var"##Language#405"[GQLInterface.var"##Language#405"(nothing, nothing, nothing, "English")]
```
## Parent Types

By default, all introspected types have the parent type [`GQLInterface.AbstractIntrospectedStruct`](@ref),
