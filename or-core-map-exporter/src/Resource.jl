...
"""
function __init__()
    APP_PORT[] =  get(ENV, "APP_PORT", "6705")
...
function execute_exporting(trigger_layer, req)
    ...
    # local msg = ""
    try
        tsk = Workers.@async App.map_export(trigger_layer)
        result = fetch(tsk)
        result isa Exception && rethrow(result)
        @info "MapExporter - Exporting successfully completed"
        code = 200
    catch err
        @error "MapExporter - Exporting failed due to " exception=(err, catch_backtrace())
        msg = "MMapExporter - Exporting failed - check logs for error details"
        code = 400
    end
    return code, msg
end
HTTP.@register(ROUTER, "/execute", execute_exporting)
