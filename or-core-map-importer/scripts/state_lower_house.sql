drop table if exists tmp_state_lower_house;
drop table if exists tmp_state_lower_house_attribute;

with tmp_state_lower_house1 as (
  select
    ...
    "ufi",
    "ufi_created"
  from map_manager.stg_state_lower_house
)
select
  'STATE_LOWER_HOUSE&&' || object_id as feature_id,  -- Maybe generate from s_source_id when we get this
  "district" as s_name,
  geom_feature,
  ...
  "ufi",
  "ufi_created"
into temporary tmp_state_lower_house
from tmp_state_lower_house1
;

with tmp_state_lower_house2 as (
  select
    feature_id || '&&Shape__Area' as attribute_id,
    feature_id as feature_id,
    'Shape__Area' as s_name,
    s_name as s_value
  from tmp_state_lower_house
  union
select
  ...
  union
select
    feature_id || '&&district_code' as attribute_id,
    feature_id as feature_id,
    'district_code' as s_name,
    s_name as s_value
  from tmp_state_lower_house
  union
select
    feature_id || '&&district_label' as attribute_id,
    feature_id as feature_id,
    'district_label' as s_name,
    s_name as s_value
  from tmp_state_lower_house
  union
select
...
select
  ...
  s_name,
  s_value
into temporary tmp_state_lower_house_attribute
from tmp_state_lower_house2
;

-- mm_layer, delete previous data
delete from map_manager.mm_feature where layer_id = 'STATE_LOWER_HOUSE';

-- mm_layer
insert into map_manager.mm_layer
select
  'STATE_LOWER_HOUSE' as layer_id,
  'State Lower House' as s_name,
...
from tmp_state_lower_house_attribute
