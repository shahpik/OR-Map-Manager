"""
    MapImporterException

A custom exception for this application.

This replaces generic use of `error` and should be used with `throw`:

```julia
throw(MapImporterException("Error message"))
```

Note, this should not replace more appropriate exception types such as
`ArgumentError` or `DimensionMismatch`.
"""
struct MapImporterException <: Exception
    msg::String
end

function Base.showerror(io::IO, ex::MapImporterException; backtrace=true)
    printstyled(io, "MapImporterException:\n\n" * ex.msg * "\n", color=Base.error_color())
end