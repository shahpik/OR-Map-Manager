...
"""
function __init__()
    APP_PORT[] =  get(ENV, "APP_PORT", "7321")
...
# MapMatcher endpoint should be updated to receive requests with a JSON payload containing sourceName and changeset_id
function execute_matching(req)
    # Converts from byte array (Vector{UInt8}) to String
    body = uppercase(String(req.body))
    local code = 200
    local msg = ""
    try
        @info "MapMatcher - Starting matching of $(body)"
        tsk = Workers.@async App.map_matching(body)
        result = fetch(tsk)
        result isa Exception && rethrow(result)
        @info "MapMatcher - Matching of $(body) successfully completed"
        code = 200
    catch err
        @error "MapMatcher - Matching of $(body) failed due to " exception=(err, catch_backtrace())
        msg = "MapMatcher - Matching of $(body) failed - check logs for error details"
        code = 400
    end
    return code, msg
end
HTTP.@register(ROUTER, "/execute", execute_matching)
