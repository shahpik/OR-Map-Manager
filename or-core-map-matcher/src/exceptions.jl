"""
    MapMatcherException

A custom exception for this application.

This replaces generic use of `error` and should be used with `throw`:

```julia
throw(MapMatcherException("Error message"))
```

Note, this should not replace more appropriate exception types such as
`ArgumentError` or `DimensionMismatch`.
"""
struct MapMatcherException <: Exception
    msg::String
end

function Base.showerror(io::IO, ex::MapMatcherException; backtrace=true)
    printstyled(io, "MapMatcherException:\n\n" * ex.msg * "\n", color=Base.error_color())
end